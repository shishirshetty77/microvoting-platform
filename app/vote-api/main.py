...
def get_redis_conn():
    ...
    try:
        r = Redis(host=redis_host, port=redis_port, db=0, socket_connect_timeout=2, decode_responses=True)
        r.ping()
...
async def submit_vote(vote: Vote):
    if not redis_conn:
        return JSONResponse(
            ...
            content={"message": "Voting service is currently unavailable. Could not connect to Redis."},
        )
